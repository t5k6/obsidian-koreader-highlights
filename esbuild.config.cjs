...
async function embedWasm() {
    try {
        ...
        if (!fs.existsSync(wasmSourcePath)) {
            console.error(
                `❌ FATAL: WASM source file not found at ${wasmSourcePath}. Ensure 'sql.js' is installed correctly.`,
            );
...
`;
        ...
        console.log(
            `✅ WASM binary embedded successfully into: ${tsOutputPath}`,
        );
...
async function build() {
    ...
    console.log(
        `Running esbuild in ${isProduction ? "production" : "development"} mode.`,
    );
...
*/`;
    ...
    const buildOptions = {
        ...
        external: [
            ...
            ...builtinModules.filter(
                (m) =>
                    !["electron", "obsidian"].includes(m) && !m.startsWith("_"),
            ),
        ],
        format: "cjs",
        target: "es2022",
    ...
    try {
        ...
        if (isProduction) {
            ...
            console.log(
                `📏 Production main.js size: ${(stats.size / 1024).toFixed(2)} KB`,
            );
        }
    } catch (error) {
