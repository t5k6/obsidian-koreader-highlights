...
import type { Disposable, KoreaderHighlightImporterSettings, SettingsObserver } from "src/types";
...
export type AnyClass<T = unknown, P extends any[] = any[]> = new (...args: P) => T;
...
function isSettingsObserver(obj: unknown): obj is SettingsObserver {
  return !!obj && typeof (obj as SettingsObserver).onSettingsChanged === "function";
}

function isDisposable(obj: unknown): obj is Disposable {
  return !!obj && typeof (obj as { dispose?: () => void | Promise<void> }).dispose === "function";
}
...
export class DIContainer {
  ...
  private readonly registrations = new Map<Token<unknown>, Registration<unknown, any[]>>();
  ...
  public register<T, P extends any[]>(
    ...
    if (this.registrations.has(token)) {
      this.log.warn(`DI registration for ${this.getTokenName(token)} is being overwritten.`);
    }
  ...
  public registerValue<T>(token: Token<T>, value: T): this {
    if (this.values.has(token)) {
      this.log.warn(`DI value for ${this.getTokenName(token)} is being overwritten.`);
    }
  ...
  public resolve<T>(token: Token<T>): T {
    ...
    if (this.resolvingStack.includes(token)) {
      const cyclePath = [...this.resolvingStack, token]
        ...
        .join(" -> ");
      throw new Error(`DIContainer: Circular dependency detected: ${cyclePath}`);
    }
    ...
    try {
      let instance: T;
      const resolvedDeps = registration.deps.map((depToken) => this.resolve(depToken));
      ...
      if (registration.factory) {
        ...
        if (!classCtor) {
          throw new Error(
            `DIContainer: Registration for ${this.getTokenName(token)} has no class or factory.`,
          );
        }
        // Instantiate the class, casting the constructor to be callable with the resolved dependencies
        instance = new (classCtor as AnyClass<T, typeof resolvedDeps>)(...resolvedDeps);
      }

      // Backfill settings for new instances if they are SettingsObservers
      if (isSettingsObserver(instance) && this.lastKnownSettings) {
        try {
          instance.onSettingsChanged(this.lastKnownSettings.new, this.lastKnownSettings.old);
        } catch (error) {
          this.log.error(`Error notifying ${this.getTokenName(token)} of initial settings.`, error);
        }
      }
