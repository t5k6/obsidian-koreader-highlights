...
export class PluginDataStore {
  ...
  public async load(): Promise<PluginData> {
    return this.mutex.lock(async () => {
      ...
      if (isErr(ensured)) {
        ...
        this.log.warn("PluginDataStore: failed to ensure plugin data dir", error);
      }
  ...
  public async save(data: PluginData): Promise<void> {
    await this.mutex.lock(async () => {
      ...
      if (isErr(ensured)) {
        // Type-safe error access
        const error = ensured.error;
        this.log.warn("PluginDataStore: failed to ensure plugin data dir before save", error);
      }

      const primaryResult = await this.fs.writePluginDataJsonAtomic(this.dataPath(), data);
      if (isErr(primaryResult)) {
        this.log.error("PluginDataStore: failed to save primary data.json", primaryResult.error);
        throw primaryResult.error;
      }

      const backupResult = await this.fs.writePluginDataJsonAtomic(this.backupPath(), data);
      if (isErr(backupResult)) {
        this.log.warn("PluginDataStore: failed to update backup", backupResult.error);
      }
  ...
  public async saveSettings(newSettings: KoreaderHighlightImporterSettings): Promise<PluginData> {
