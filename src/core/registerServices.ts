 ...
 */
export function registerServices(container: DIContainer, plugin: KoreaderImporterPlugin, app: App) {
  ...
  container.registerValue(
    DUPLICATE_MODAL_FACTORY_TOKEN,
    (app: App, match: DuplicateMatch, message: string, session: DuplicateHandlingSession) =>
  ...
  container.register(NoteEditorService, [APP_TOKEN, LoggingService, FileSystemService]);

  // --- Level 1: Depends on Level 0 or Tokens ---
  container.register(FileSystemService, [VAULT_TOKEN, PLUGIN_TOKEN, CacheManager, LoggingService]);
  ...
  container.register(StatusBarManager, [APP_TOKEN, PLUGIN_TOKEN, IndexCoordinator, CommandManager]);
  container.register(IndexRebuildStatusService, [IndexDatabase, LoggingService]);
  ...
  // Index components
  container.register(IndexDatabase, [SqlJsManager, FileSystemService, LoggingService]);
