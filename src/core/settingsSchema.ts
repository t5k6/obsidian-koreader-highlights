...
export const TemplateSchema = z
  .object({
    ...
    noteQuotingMode: z.enum(["auto", "manual", "never"]).catch("auto").default("auto"),
  })
  .passthrough();
...
export const FrontmatterSchema = z
  .object({
    ...
    keywordsAsTags: z.enum(["none", "duplicate", "replace"]).catch("none").default("none"),
    durationFormat: z.enum(["formatted", "seconds"]).catch("formatted").default("formatted"),
  })
  .passthrough();
...
export function normalizeSettings(raw: unknown): KoreaderHighlightImporterSettings {
  ...
  merged.koreaderScanPath = Pathing.normalizeSystemPath(merged.koreaderScanPath);
  ...
  merged.template.templateDir = Pathing.toVaultPath(merged.template.templateDir);
  ...
  merged.excludedFolders = merged.excludedFolders.map((f) => String(f).trim()).filter(Boolean);
...
export const DEFAULT_SETTINGS: KoreaderHighlightImporterSettings = normalizeSettings({});
...
export function normalizePluginData(raw: unknown): PluginData {
  ...
  return {
    schemaVersion:
      typeof rawObj.schemaVersion === "number" && Number.isInteger(rawObj.schemaVersion)
    ...
    appliedMigrations: Array.isArray(rawObj.appliedMigrations)
      ? rawObj.appliedMigrations.filter((x): x is string => typeof x === "string")
      : [],
    lastPluginMigratedTo:
      typeof rawObj.lastPluginMigratedTo === "string" ? rawObj.lastPluginMigratedTo : undefined,
  };
}
