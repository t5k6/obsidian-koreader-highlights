...
export class CacheManager implements Disposable {
  ...
  public clearWhere(pred: (name: string, cache: Cache<unknown, unknown>) => boolean): number {
    let clearedCount = 0;
    for (const [name, cache] of this.registry) {
      if (pred(name, cache)) {
        try {
  ...
  public createTtlCache<K, V>(name: string, ttlMs: number): IterableCache<K, V> {
    const cache = new SimpleCache<K, V>(undefined, ttlMs);
    return this.register(name, cache);
  }

  public createTtlLru<K, V>(name: string, max: number, ttlMs: number): IterableCache<K, V> {
