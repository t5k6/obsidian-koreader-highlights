...
export async function withFsRetry<T>(fn: () => Promise<T>, opts?: RetryOptions): Promise<T> {
  return retry(fn, {
    ...FS_RETRY_DEFAULTS,
    ...opts,
...
function computeDelay(
  attempt: number,
  cfg: Required<Pick<RetryOptions, "baseDelayMs" | "maxDelayMs" | "factor" | "jitter">>,
): number {
  const { baseDelayMs, maxDelayMs, factor } = cfg;
  let delay = Math.min(maxDelayMs, baseDelayMs * factor ** (attempt - 1));
  const j = cfg.jitter === true ? "full" : cfg.jitter === false ? "none" : cfg.jitter;
  if (j === "full") {
