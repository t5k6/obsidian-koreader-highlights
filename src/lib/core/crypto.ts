...
export function canonicalize(
  ...
  const { normalizeEol = true, trim = false, collapseWhitespace = false } = opts;
...
export async function sha256HexAsync(
  ...
  try {
    ...
    return err(
      new Error("Crypto fallback failed: Node.js crypto module is unavailable or threw an error."),
    );
  }
}
 ...
 * @returns A `Result` containing the hex string on success, or an `Error` on failure.
 */
export async function sha1HexOfBuffer(buf: ArrayBuffer): Promise<Result<string, Error>> {
  try {
