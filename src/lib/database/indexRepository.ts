...
export class IndexRepository {
  ...
  async deleteNoteAndResetSource(notePath: string): Promise<string | null> {
    ...
    return concurrentDb.writeTx((d: Database) => {
      ...
      if (bookKey) {
        ...
        const [sourcePath] = executeTyped(d, latestSourceQuery, RowMappers.sourcePath);

        if (sourcePath) {
          // Delete the import source entry (reset status)
          const deleteSourceQuery = QueryBuilders.deleteImportSourceByPath(sourcePath);
  ...
  async upsertFromMetadata(
    ...
    return concurrentDb.writeTx((d: Database) => {
      ...
      const upsertQueries = [
        QueryBuilders.upsertBook(metadata.key, null, metadata.title, metadata.authors),
  ...
  async deleteInstanceForFile(
    ...
    return concurrentDb.writeTx((d: Database) => {
      ...
      if (oldKey) {
        const deleteInstanceQuery = QueryBuilders.deleteInstanceByPath(filePath);
        executeWrite(d, deleteInstanceQuery);
  ...
  async recordImportSuccess(params: {
    ...
    await concurrentDb.writeTx((d: Database) => {
      ...
      if (params.vaultPath && params.bookKey) {
        ...
        const upsertBook = QueryBuilders.upsertBook(book.key, book.id, book.title, book.authors);
        const upsertInstance = QueryBuilders.upsertInstance(book.key, params.vaultPath);
   ...
   */
  async upsertBookWithInstance(book: BookRow, vaultPath?: string): Promise<void> {
    const concurrentDb = await this.getConcurrentDb();
    await concurrentDb.writeTx((d: Database) => {
      const queries = [
  ...
  async handleRenameFolder(oldPath: string, newPath: string): Promise<void> {
    const concurrentDb = await this.getConcurrentDb();
    await concurrentDb.writeTx((d: Database) => {
      const q = QueryBuilders.renameFolder(`${oldPath}/`, `${newPath}/`, `${oldPath}/%`);
      executeWrite(d, q);
    });
  }
  ...
  static shouldProcess(
    ...
    if (existing.last_error !== null || existing.last_success_ts === null) return true;
    ...
    if (existing.md5 && md5) {
      ...
      return !!(newestAnnotationTs && newestAnnotationTs > (existing.newest_annotation_ts ?? ""));
    }
