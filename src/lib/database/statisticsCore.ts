...
export function calculateReadingStreak(sessions: PageStatData[]): number {
  ...
  for (let i = 1; i < uniqueDays.length; i++) {
    ...
    const expectedDay = getLocalDateKey(Math.floor(currentDate.getTime() / 1000));
 ...
 */
export function calculateAverageSessionDuration(pageReads: PageStatData[]): number {
...
export function calculateDerivedStatistics(
  ...
  const firstReadDate = firstStartSec > 0 ? new Date(firstStartSec * 1000) : null;
  ...
  const readingStatus: ReadingStatus =
    percentComplete >= 100 ? "completed" : sessions.length === 0 ? "unstarted" : "ongoing";
 ...
 */
export function getSessions(db: Database, bookId: number, signal?: AbortSignal): PageStatData[] {
  throwIfAborted(signal);
