...
import type { Annotation, DocProps, DuplicateMatch, LuaMetadata } from "src/types";
...
export function filenameMatchesKeys(
  ...
  const stem = stemWithMetadata.replace(/(?:\s*(?:\[[^\]]*]|\([^)]*\)))+$/g, "");
...
export function analyzeAnnotations(
  ...
  const byKey = new Map<string, Annotation>(existing.map((h) => [getHighlightKey(h), h]));
  ...
  for (const n of incoming) {
    ...
    if (!prev) {
      ...
      const prevNormText = Pathing.normalizeWhitespace(prev.text ?? "").toLowerCase();
      const incNormText = Pathing.normalizeWhitespace(n.text ?? "").toLowerCase();

      const prevNormNote = Pathing.normalizeWhitespace(prev.note ?? "").toLowerCase();
      const incNormNote = Pathing.normalizeWhitespace(n.note ?? "").toLowerCase();
