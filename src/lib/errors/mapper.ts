...
export function getFsCode(e: unknown): string | undefined {
  ...
  const msg: string | undefined = typeof maybe?.message === "string" ? maybe.message : undefined;
  if (msg) {
...
export function toFailure(
  ...
  if (message.includes("already exists")) return { kind: "AlreadyExists", path };
  ...
  if (message.includes("not a directory")) return { kind: "NotADirectory", path };
