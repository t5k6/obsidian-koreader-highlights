 ...
 */
const FIELD_FORMATTERS: Record<keyof FrontmatterData, FieldFormatter | undefined> = {
  lastRead: (rawValue) => {
    const r = formatDateResult(rawValue as string | number | Date, "{YYYY}-{MM}-{DD}");
    return r.ok ? r.value : "";
  },
  firstRead: (rawValue) => {
    const r = formatDateResult(rawValue as string | number | Date, "{YYYY}-{MM}-{DD}");
  ...
  readTime: (rawValue, opts) => {
    ...
    return opts.durationFormat === "seconds" ? seconds : formatDurationHms(seconds);
  },
  ...
  keywords: (rawValue) => {
    ...
    const arr = Array.isArray(rawValue) ? rawValue : splitAndTrim(String(rawValue), /,/);
...
function shouldIncludeField(
  ...
  if ((key === "highlightCount" || key === "noteCount") && value === 0) return false;
  return true;
}
...
export function formatForDisplay(
  ...
  const disabledFields = new Set(opts.disabledFields.map((f) => f.toLowerCase()));
