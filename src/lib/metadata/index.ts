 ...
 */
export function normalizedToFrontmatter(normalized: NormalizedMetadata): FrontmatterData {
  ...
  if (Array.isArray(result.authors)) {
    result.authors = result.authors.length === 1 ? result.authors[0] : result.authors;
  }
