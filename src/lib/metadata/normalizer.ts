...
export function normalizeBookMetadata(raw: LuaMetadata): NormalizedMetadata {
  ...
  const authors = authorsStr ? finalizeStringList(splitOnDelimiters(authorsStr)) : [];
  ...
  const keywords = keywordsStr ? finalizeStringList(splitOnDelimiters(keywordsStr)) : [];
  ...
  if (isLuaComplete) {
    ...
    result.readingStatus = luaStatus === "complete" ? "completed" : raw.luaSummary.status;
  }
...
export function parseAuthors(value: unknown): string[] {
  if (Array.isArray(value)) {
    ...
    return finalizeStringList(splitOnDelimiters(value).map((author) => parseAuthor(author)));
 ...
 */
export function normalizeFrontmatter(fm: Record<string, unknown>): NormalizedMetadata {
  ...
  const normalizedStatus = rawStatus?.toLowerCase().trim() === "complete" ? "completed" : rawStatus;
...
export function mergeNormalizedMetadata(
  ...
  if ((!merged.keywords || merged.keywords.length === 0) && base.keywords?.length) {
  ...
  if (merged.totalReadSeconds === undefined && base.totalReadSeconds !== undefined) {
  ...
  if (merged.avgSessionDuration === undefined && base.avgSessionDuration !== undefined) {
  ...
  } else if (base.lastRead && merged.lastRead && base.lastRead > merged.lastRead) {
    merged.lastRead = base.lastRead;
  }

  // Average time per page: Keep if incoming doesn't have it
  if (merged.averageTimePerPage === undefined && base.averageTimePerPage !== undefined) {
  ...
  if (merged.highlightCount === undefined && base.highlightCount !== undefined) {
