...
type SnapshotState = { kind: "valid"; content: string } | { kind: "empty" } | { kind: "missing" };
...
export function prepareForReplace(
  ...
  const updater: NoteUpdater = (currentDoc: NoteDoc) => {
    ...
    const mergedMeta = mergeNormalizedMetadata(currentMeta, newMeta, fmSettings);

    const displayFm = formatForDisplay(mergedMeta, fmSettings);
    if (currentDoc.frontmatter[KOHL_UID_KEY]) {
      (displayFm as any)[KOHL_UID_KEY] = currentDoc.frontmatter[KOHL_UID_KEY] as string;
    }
...
export function prepareForMerge(params: {
  ...
  const { baseSnapshotBody, incomingBody, lua, settings, logger = NO_OP_LOGGER } = params;
  ...
  if (snapshotState.kind === "valid") {
    ...
    logger.warn(`prepareForMerge: Snapshot issue (${snapshotState.kind}). using empty base.`);
    effectiveBaseBody = "";
  ...
  const updater: NoteUpdater = (currentDoc: NoteDoc) => {
    ...
    const currentMeta = normalizeFrontmatter(currentDoc.frontmatter);
    const newMeta = normalizeBookMetadata(lua);
    const mergedMeta = mergeNormalizedMetadata(currentMeta, newMeta, settings.frontmatter);
