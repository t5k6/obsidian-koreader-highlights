...
import type { Annotation, DocProps, DrawerType, LuaMetadata, PositionObject } from "src/types";
...
const sanitizeString = (raw: string): string => {
  ...
  // We use "\n\n" here because KOReader exports paragraph breaks in highlights
  // as escaped line continuations, and users want spacing between paragraphs
  // when rendered as Markdown.
  const withLineContinuationsAsNewlines = sliceQuotes(raw).replace(
    /\\(\r?\n)/g,
    "\n\n", // Replace line continuation with double newline
  );
...
function evaluateTable(table: TableConstructorExpression): unknown {
  ...
  const isDense = maxIntegerIndex > 0 && integerKeyCount >= maxIntegerIndex * 0.5;
...
const MetadataRootSchema = z.preprocess(
  ...
  z
    .object({
      ...
      annotations: z.union([z.array(z.any()), z.record(z.string(), z.any())]).optional(),
      highlight: z.union([z.array(z.any()), z.record(z.string(), z.any())]).optional(),
    })
    .passthrough(),
);
...
function mapDocProps(raw: z.infer<typeof DocPropsSchema> | undefined): DocProps {
  ...
  if (raw.keywords) {
    base.keywords = raw.keywords.replace(/\\?\n/g, ", ").replace(/,\s*,/g, ",").trim();
  }
...
const toDrawerType = (s: string): DrawerType | null => {
  ...
  return (DRAWER_TYPES as readonly string[]).includes(v) ? (v as DrawerType) : null;
};
...
function mapAnnotation(raw: unknown, pageFallback: number): Annotation | null {
  ...
  const pageno = typeof normalized.pageno === "number" ? normalized.pageno : pageFallback;

  return {
    text,
    note: String(note),
    pageno,
    pageref: typeof normalized.pageref === "string" ? normalized.pageref : undefined,
    chapter: typeof normalized.chapter === "string" ? normalized.chapter : undefined,
    color: typeof normalized.color === "string" ? normalized.color.trim() : undefined,
    drawer,
    datetime:
      typeof normalized.datetime === "string" ? normalized.datetime : new Date().toISOString(),
...
function normalizeAnnotations(rawList: unknown): Annotation[] {
  ...
  const traverse = (node: unknown, pageContext: number) => {
    ...
    for (const [key, value] of Object.entries(node as Record<string, unknown>)) {
      ...
      const nextPageContext = Number.isInteger(numKey) && numKey > 0 ? numKey : pageContext;
...
/**
 * KOReader keeps the same highlights in `annotations` and in the legacy
 * page-keyed `highlight` table. Merging both without deduping duplicates every
 * entry. Stable span identity is pos0 + pos1 (XPointers).
 */
function positionKey(pos: unknown): string {
  if (pos == null) return "";
  if (typeof pos === "string") return pos;
  if (typeof pos === "object" && pos !== null && "x" in pos && "y" in pos) {
    const o = pos as PositionObject;
    return `${o.x},${o.y}`;
  }
  try {
...
function annotationDedupeKey(a: Annotation): string {
  const p0 = positionKey(a.pos0);
  const p1 = positionKey(a.pos1);
  if (p0 || p1) return `${p0}\0${p1}`;
  return `${a.pageno}\0${a.datetime}\0${a.text ?? ""}`;
}

function dedupeAnnotationsInOrder(anns: Annotation[]): Annotation[] {
  const seen = new Set<string>();
  const out: Annotation[] = [];
  for (const a of anns) {
    const key = annotationDedupeKey(a);
...
export function hasAnnotations(luaCode: string): boolean {
  ...
  const emptyAnnotationsPattern = /(?:\["annotations"\]|annotations)\s*=\s*\{\s*\}/;

  // Has annotations table but not empty
  return annotationsPattern.test(luaCode) && !emptyAnnotationsPattern.test(luaCode);
}
...
export function parse(luaContent: string): Result<ParseSuccess, ParseFailure> {
  ...
  const returns = (ast.body ?? []).filter((n: any) => n && n.type === "ReturnStatement");
  ...
  const merged = [...modernAnns, ...legacyAnns].filter((a) => a.text?.trim());
  meta.annotations = dedupeAnnotationsInOrder(merged);
