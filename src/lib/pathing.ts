 ...
 */
export function toFileSafe(input: string | null | undefined, opts?: FileSafeOptions): string {
 ...
 * This is a pure function with no caching.
 */
export function toMatchKey(input: string | null | undefined, opts?: MatchKeyOptions): string {
...
export function vaultDirname(vaultPath: VaultPath | string): VaultPath {
  const normalized = typeof vaultPath === "string" ? toVaultPath(vaultPath) : vaultPath;
...
export function isAncestor(ancestor: VaultPath | string, child: VaultPath | string): boolean {
  const ancestorNorm = typeof ancestor === "string" ? toVaultPath(ancestor) : ancestor;
...
export function simplifySdrName(raw: string, delimiter = " - "): string {
  ...
  while (changed) {
    changed = false;
    for (let blockLen = Math.floor(tokens.length / 2); blockLen >= 1; blockLen--) {
      for (let i = 0; i <= tokens.length - 2 * blockLen; i++) {
        ...
        if (block1.every((val, index) => val.toLowerCase() === block2[index].toLowerCase())) {
...
export function generateFileName(
  ...
  if (options.useCustomTemplate) {
    ...
    baseName = options.template.replace(/\{\{(\w+)\}\}/g, (_, key) => templateData[key] || "");
  } else {
    baseName = generateDefaultBaseName(docProps, originalSdrName);
  }

  if (!baseName?.trim()) {
    baseName = simplifySdrName(getFileNameWithoutExt(originalSdrName)) || "Untitled";
  }
...
function generateDefaultBaseName(docProps: DocProps, originalSdrName?: string): string {
...
export function validateFileNameTemplate(template: string): {
  ...
  if (result.isValid && !placeholders.includes("title") && !placeholders.includes("authors")) {
    result.warnings.push(
...
export function computeStemBudget(
  ...
  const fixedLen =
    (fixedParts ? fixedParts.length + 1 : 0) + extensionWithDot.length + suffixReserve;
 ...
 */
export function truncateWithHash(stem: string, budget: number, hashLength: number): string {
...
export async function generateUniqueStem(
  ...
  if (baseStem.length > budget) {
    baseStem = truncateWithHash(baseStem, budget, FILENAME_TRUNCATION_HASH_LENGTH);
    wasTruncated = true;
  }
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    const candidatePath = joinVaultPath(baseDir, `${candidateStem}${extensionWithDot}`);
  ...
  const finalStem = truncateWithHash(fallbackStem, budget, FILENAME_TRUNCATION_HASH_LENGTH);
...
export function systemDirname(systemPath: SystemPath | string): SystemPath {
  const normalized = typeof systemPath === "string" ? normalizeSystemPath(systemPath) : systemPath;
  return path.dirname(normalized) as SystemPath;
}

export function systemBasename(systemPath: SystemPath | string, ext?: string): string {
  const normalized = typeof systemPath === "string" ? normalizeSystemPath(systemPath) : systemPath;
  return path.basename(normalized, ext);
}

export function systemRelative(from: SystemPath | string, to: SystemPath | string): string {
  const normalizedFrom = typeof from === "string" ? normalizeSystemPath(from) : from;
