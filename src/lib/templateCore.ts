...
import { escapeHtml, escapeMarkdown, stripHtml, unescapeHtml } from "src/lib/strings/stringUtils";
import type { Annotation, CommentStyle, RenderContext, TemplateData } from "src/types";
...
export const TEMPLATE_FILTERS = {
  ...
  truncate: {
    ...
    apply: (s: string, arg?: string): string => {
      const n = Number.isFinite(Number(arg)) ? Number(arg) : parseInt(String(arg ?? ""), 10);
      return Number.isFinite(n) && n > 0 && s.length > n ? s.slice(0, n) + "…" : s;
    },
  },
  br2nl: {
...
export function validateTemplate(template: string): TemplateValidationResult {
  ...
  if (!hasHighlight) {
    errors.push("Missing required variable: {{highlight}} or {{highlightPlain}} ");
    suggestions.push("Add {{highlight}} for styled text or {{highlightPlain}} for plain text");
  }
  if (!vars.has("pageno")) {
...
function mergeHighlightText(group: Annotation[], separators: (" " | " [...] ")[]): string {
  const styled = group.map((ann) => styleHighlight(ann.text ?? "", ann.color, ann.drawer));
  return joinBlocks(styled, separators);
}

function mergeHighlightTextPlain(group: Annotation[], separators: (" " | " [...] ")[]): string {
...
/**
 * Normalizes template whitespace at compile time.
 * Only removes trailing empty lines from template to prevent accumulation.
 */
function normalizeTemplateWhitespace(template: string): string {
  return template.replace(/\n+\s*$/g, "\n");
}

/**
 * Normalizes rendered output by removing leading/trailing whitespace.
 * Does NOT collapse internal newlines - preserves semantic paragraph breaks.
 */
function normalizeOutput(rendered: string): string {
  if (!rendered) return "";
  // Only remove leading/trailing whitespace - never touch internal structure
...
export function renderGroup(
  ...
  const data: TemplateData = {
    ...
    highlight: mergeHighlightText(group, ctx.separators ?? new Array(group.length - 1).fill(" ")),
    highlightPlain: mergeHighlightTextPlain(
      group,
      ctx.separators ?? new Array(group.length - 1).fill(" "),
    ),
    note: mergeNotes(group),
    notes: group.map((g) => g.note).filter((note): note is string => typeof note === "string"),
    color,
    drawer: head.drawer,
...
export function renderAnnotations(
  ...
  for (const chapter of chapters) {
    ...
    const groups = groupSuccessiveHighlights(chapter.annotations, maxHighlightGap);
    let isFirst = true;
    for (const g of groups) {
      ...
      // preserve internal paragraph breaks
      if (!rendered || rendered.trim() === "") continue;

      // Add KOHL markers if needed
      const block =
        commentStyle !== "none"
  ...
  // Only join with double newline - this creates separation between highlights
  // without touching internal paragraph structure
  return blocks.join("\n\n");
}

export function tokenize(template: string, maxDepth = MAX_TEMPLATE_NESTING): TemplateToken[] {
...
export function compile(
  ...
  // Pre-process template to normalize excessive whitespace at compile time
  const normalizedTemplate = normalizeTemplateWhitespace(templateString);
  const tokens = tokenize(normalizedTemplate);
  ...
  const renderTokens = (ts: TemplateToken[], data: TemplateData): string => {
    let out = "";
    let lastTokenWasCond = false;

    for (const t of ts) {
      if (t.type === "text") {
        // Skip empty text tokens that immediately follow conditional blocks
        // to avoid whitespace from unrendered conditionals
        if (lastTokenWasCond && t.value.trim() === "") {
          continue;
        }
        out += t.value;
        lastTokenWasCond = false;
      } else if (t.type === "var") {
        const raw =
          t.key === "note" ? renderNote((data as any).note, quotingStyle) : (data as any)[t.key];
        const filtered = applyFilters(raw, t.filters, { cache });

        // Skip empty variable output to reduce unnecessary whitespace
        if (filtered.trim() === "") {
          lastTokenWasCond = false;
          continue;
        }
        ...
        if ((data as any)[t.key]) {
          out += renderTokens(t.body, data);
          lastTokenWasCond = true;
        } else {
          // Mark that we skipped a conditional block
          lastTokenWasCond = true;
        }
      }
    }
  ...
  return (data: TemplateData) => {
    const rendered = renderTokens(tokens, data);
...
export function joinBlocks(blocks: string[], separators: (" " | " [...] ")[]): string {
