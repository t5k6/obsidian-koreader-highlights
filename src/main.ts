...
export default class KoreaderImporterPlugin extends Plugin {
  ...
  async onload() {
    ...
    if (this.servicesInitialized) {
      ...
      this.loggingService.info("KoreaderImporterPlugin", "UI components loaded.");
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      this.loggingService.info("KoreaderImporterPlugin", "All services initialized successfully.");
    } catch (error) {
      this.servicesInitialized = false;
      const errorMessage =
  ...
  private async phase<T>(name: string, fn: () => Promise<T> | T): Promise<T> {
    ...
    try {
      ...
      this.loggingService?.error("KoreaderImporterPlugin", `Phase "${name}" failed`, e);
      throw e;
    }
  }
  ...
  private async initPersistenceAndMigrations(): Promise<void> {
    ...
    await this.migrationManager.migrateLegacySettingsIfNeeded(this, this.dataStore);
    ...
    const localIndexService = this.diContainer.resolve<IndexCoordinator>(IndexCoordinator);
    ...
    // Schedule deferred migrations to run after UI is ready
    this.app.workspace.onLayoutReady(() => {
      void this.migrationManager.runDeferred(this.dataStore, {
        notePersistenceService,
        localIndexService,
  ...
  private async initCoreServices(): Promise<void> {
    this.localIndexService = this.diContainer.resolve<IndexCoordinator>(IndexCoordinator);
    // Eager init for perf; IndexCoordinator.whenReady() will lazy-init if needed
    await this.localIndexService.initialize();

    this.templateManager = this.diContainer.resolve<TemplateManager>(TemplateManager);
    await this.templateManager.loadBuiltInTemplates();
    await this.templateManager.ensureTemplates();
    this.loggingService.info("KoreaderImporterPlugin", "Templates initialized.");
  }

  private initUIManagers(): void {
    this.statusBarManager = this.diContainer.resolve<StatusBarManager>(StatusBarManager);
    this.addChild(this.statusBarManager);

    // Initialize UI services that subscribe to state changes
    const rebuildStatusService =
      this.diContainer.resolve<IndexRebuildStatusService>(IndexRebuildStatusService);
  ...
  private registerFileMenuHandler(): void {
          ...
          if (isTFile(file)) {
            menu.addItem((item) => {
              item
                ...
                .onClick(async () => {
                  void this.runAndNotify(commandManager.executeFinalizeCurrentNote(file), {
                    onSuccess: (result) => {
                      new Notice(
                        result.changed
            ...
            menu.addItem((item) => {
              item
                .setTitle("Restore KOReader backup")
                .setIcon("history")
                .onClick(() => {
                  void commandManager.executeRestoreFromBackup(file);
                });
            });
              ...
              item
                ...
                .onClick(async () => {
                  ...
                  if (confirmed) {
                    ...
                    new Notice("Folder finalization not yet implemented.", 4000);
                  }
                });
            });
          }
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        ...
        if (!file || !Pathing.isAncestor(this.settings.highlightsFolder, file.path)) {
          return;
        }
  ...
  private async runAndNotify<T>(
    ...
    const res = await notifyOnError(p, {
      message: (err) => formatError(err),
      timeout: 10000, // Show for 10 seconds to give user time to read.
    });
    if (!isErr(res) && options?.onSuccess) {
