...
export class AsyncFileSink {
  ...
  constructor(fs: FileSystemService, dir: string, retentionFiles = 10, flushDelayMs = 800) {
    this.fs = fs;
    this.dir = dir;
    this.retentionFiles = retentionFiles;
    this.flushDebounced = debounce(() => void this.flushNow(), flushDelayMs, false);
  ...
  private async enforceRetention(): Promise<void> {
    ...
    const files = listed.value.files
      .filter((p) => /\/log_\d{4}-\d{2}-\d{2}\.md$/.test(Pathing.toVaultPath(p)))
