...
function _shouldInvalidateScanCache(changedPath: VaultPath, key: ScanCacheKey): boolean {
...
export class FileSystemService {
  ...
  public async *iterateVaultFiles(
    ...
    const filter = opts?.extensions?.length ? createExtensionFilter(opts.extensions) : null;
  ...
  public getPluginDataDir(): VaultPath {
    ...
    const cfg = (this.vault as Vault & { configDir?: string }).configDir ?? ".obsidian";
  ...
  private async probeRenameReplaceSupport(): Promise<boolean> {
    if (this.renameReplaceSupported !== null) return this.renameReplaceSupported;
    if (this.renameReplaceProbePromise) return this.renameReplaceProbePromise;

    this.renameReplaceProbePromise = this.keyedQueue.run("__probe_rename_replace__", async () => {
      if (this.renameReplaceSupported !== null) return this.renameReplaceSupported;
      ...
      if (this.renameReplaceSupported === false && !this.loggedCapabilityOnce) {
        this.log.info("Adapter does not support rename-over-existing; using swap fallback.");
   ...
   * Result-based API.
   */
  public async readVaultBinary(vaultPath: string): Promise<Result<Uint8Array, AppFailure>> {
  ...
  public async writeVaultBinaryAtomic(
    ...
    return this.keyedQueue.run(`atomic:${dst}`, async () => {
      ...
      try {
        ...
        const preCommitCheck = await verifyWrittenFile(this.vault.adapter, tmp, arrayBuffer);
  ...
  public async appendVaultText(vaultPath: string, text: string): Promise<Result<void, AppFailure>> {
    const p = Pathing.toVaultPath(vaultPath);
    const ensured = await this.ensureParentDirectory(p);
    if (isErr(ensured)) return ensured;
    ...
    return this.keyedQueue.run(`append:${p}`, async () => {
      try {
        if (typeof this.vault.adapter.append === "function") {
          await withFsRetry(
            () =>
              (this.vault.adapter.append as NonNullable<typeof this.vault.adapter.append>)(p, text),
            {
              maxAttempts: 6,
              baseDelayMs: 40,
            },
          ...
          if (isErr(existing)) {
            ...
            await withFsRetry(() => this.vault.adapter.write(p, existing.value + text), {
              maxAttempts: 6,
              baseDelayMs: 40,
            });
          }
  ...
  public async readVaultText(vaultPath: string): Promise<Result<string, AppFailure>> {
   ...
   */
  public async readVaultTextWithRetry(file: TFile): Promise<Result<string, AppFailure>> {
    try {
  ...
  async readBinaryAuto(filePath: string): Promise<Result<Uint8Array, AppFailure>> {
  ...
  public async vaultExists(vaultPath: string): Promise<Result<boolean, AppFailure>> {
    const p = Pathing.toVaultPath(vaultPath);
    try {
  ...
  public async readPluginDataText(fileName: string): Promise<Result<string, AppFailure>> {
  ...
  public async tryReadPluginDataJson<T = unknown>(fileName: string): Promise<T | null> {
  ...
  public async removePluginDataPath(relPath: string): Promise<Result<void, AppFailure>> {
    const p = this.joinPluginDataPath(relPath);
    return this.removeVaultPath(p);
  }

  public async ensureVaultFolder(folderPath: string): Promise<Result<void, AppFailure>> {
  ...
  public async ensureParentDirectory(filePath: string): Promise<Result<void, AppFailure>> {
  ...
  private async ensureAdapterFolder(vaultRelPath: string): Promise<Result<void, AppFailure>> {
      ...
      try {
        ...
        if (exists) {
          if (typeof this.adapter.stat === "function") {
            ...
            if (stat?.type === "file") return err({ kind: "NotADirectory", path: normalized });
          } else {
  ...
  public async createVaultFileUnique(
    ...
    return this.keyedQueue.run(`file:${dir}/${safeStem}`, async () => {
      ...
      const existsCheck = async (candidatePath: VaultPath): Promise<boolean> => {
      ...
      const { stem: finalStem, wasTruncated } = await Pathing.generateUniqueStem(
        safeStem,
        existsCheck,
        {
          baseDir: dir,
      ...
      const finalPath = Pathing.joinVaultPath(dir, `${finalStem}.${ext.replace(/^\./, "")}`);
          ...
          ? [
              {
                ...
                message: "A filename was too long and has been automatically shortened.",
              },
            ]
          : [];
   ...
   */
  public async previewUniqueStem(baseDir: string, desiredStem: string): Promise<string> {
    const dir = Pathing.toVaultPath(baseDir);
    ...
    const checkVaultExistence = async (candidatePath: VaultPath): Promise<boolean> => {
      const result = await this.vaultExists(candidatePath);
      return !isErr(result) && result.value;
    };

    // Delegate to the pure function
    const { stem } = await Pathing.generateUniqueStem(safeStem, checkVaultExistence, {
      baseDir: dir,
      ext: "md",
    });
  ...
  public async readNodeFileText(filePath: string): Promise<Result<string, FileSystemFailure>> {
    try {
  ...
  public async deleteNodeFile(filePath: string): Promise<Result<void, FileSystemFailure>> {
    try {
  ...
  ): AsyncIterable<Result<{ path: string; dirent: import("node:fs").Dirent }, FileSystemFailure>> {
   ...
   */
  public async moveVaultPath(src: string, dst: string): Promise<Result<void, AppFailure>> {
   ...
   */
  public async statVaultPath(vaultPath: string): Promise<{ mtime: number } | null> {
    const p = Pathing.toVaultPath(vaultPath);
    try {
  ...
  public getVaultAbsoluteBasePath(): string | null {
    try {
      ...
      if (typeof this.adapter.getFullPath === "function") {
        ...
        return typeof probe === "string" ? String(probe).replace(/[/]+.?$/, "") : null;
      }
    } catch {
      // ignore
    }
  ...
  private async _atomicMove(src: string, dst: string): Promise<Result<void, AppFailure>> {
  ...
  private handleMkdirError(error: unknown, path: string): Result<void, AppFailure> {
