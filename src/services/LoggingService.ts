...
import type { Disposable, KoreaderHighlightImporterSettings, SettingsObserver } from "../types";
...
export class LoggingService implements SettingsObserver, Disposable {
  ...
  private emit(
    ...
    const line = LogFormatters.formatLogLine(timestamp, tag, this.LOG_PREFIX, scope, msg);
  ...
  private _updateSinkFromSettings(): void {
    ...
    if (logToFile) {
      ...
      void (async () => {
        ...
        this.info("LoggingService", `File logging enabled at ${desiredLogDir}.`);
      })();
    }
  }
}
