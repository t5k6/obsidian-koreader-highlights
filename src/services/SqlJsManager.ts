...
export class SqlJsManager implements Disposable {
  ...
  private openInFlight = new Map<string, Promise<Result<Database, AppFailure>>>();
  ...
  constructor(
    ...
    this.getSqlJsLazy = asyncLazy<SqlJsStatic>(() => this.createSqlJsInstance());
  }
  ...
  public async openDatabase(
    ...
    const openPromise = (async (): Promise<Result<Database, AppFailure>> => {
      ...
      if (!bytes) {
        this.log.info(`Creating new DB with schema v${INDEX_DB_VERSION} at: ${filePath}`);
        db.run("BEGIN;");
        try {
          ...
          db.run("ROLLBACK;");
          this.log.error(`Failed to bootstrap new database: ${filePath}`, e as any);
          throw e;
        }
      }
      ...
      if (options.validate) {
        try {
          ...
          this.log.error(`Database validation failed for ${filePath}. It may be corrupt.`, e);
  ...
  public async persistDatabase(filePath: string): Promise<Result<void, AppFailure>> {
    ...
    if (path.isAbsolute(filePath)) {
      const w = await this.fsService.writeBinaryAuto(filePath, new Uint8Array(buffer));
      if (isErr(w)) {
        this.log.error(`Failed to persist database: ${filePath}`, w.error);
        return err(w.error);
      }
    } else {
      const w = await this.fsService.writeVaultBinaryAtomic(filePath, new Uint8Array(buffer));
      if (isErr(w)) {
        this.log.error(`Failed to persist database: ${filePath}`, w.error);
        return err(w.error);
      }
