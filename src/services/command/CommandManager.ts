...
import { type AppFailure, type AppResult, formatAppFailure } from "src/lib/errors/types";
import { convertKohlMarkers } from "src/lib/kohlMarkers";
import { hasAnnotations, parse as parseMetadata } from "src/lib/parsing/luaParser";
...
import { RestoreBackupModal } from "src/ui/RestoreBackupModal";
...
export class CommandManager {
  ...
  private async notifyCmd<D>(
    ...
    try {
      const res = await notifyOnError(operation, {
        message:
  ...
  public getCommands(): Command[] {
    return [
      ...
      },
      {
        ...
        callback: () => {
          void (async () => {
            const res = await this.notifyCmd(this.executeConvertCommentStyle(), {
        ...
        callback: () => {
          void (async () => {
            const res = await this.notifyCmd(this.executeClearCaches(), {
              message: () => "Failed to clear caches. Check console for details.",
            });
            if (res.status === "success") {
      ...
      },
      {
        ...
        callback: () => {
          void (async () => {
            ...
            if (res.status === "success") {
              new Notice("KOReader Importer has been reset. Reloading plugin now...", 5000);
            }
          })();
        },
      },
        ...
        callback: () => {
          void (async () => {
            const res = await this.notifyCmd<{ message?: string }>(
              ...
              {
                message: () => "Failed to re-check capabilities. See console for details.",
                timeout: 7000,
              },
            );
      ...
      },
      {
        ...
        checkCallback: (checking) => {
          ...
          if (!checking) {
            void this.notifyCmd<{ changed: boolean }>(this.executeRefreshCurrentNote(file), {
      ...
      },
      {
        ...
        callback: () => {
          void (async () => {
            const res = await this.notifyCmd(this.executeCleanupOrphanedData(), {
      ...
      },
      {
        ...
        checkCallback: (checking) => {
          ...
          if (!Pathing.isAncestor(this.plugin.settings.highlightsFolder, file.path)) return false;
          if (!checking) {
            void this.notifyCmd<{ changed: boolean }>(this.executeFinalizeCurrentNote(file), {
      ...
      {
        id: "restore-note-from-backup",
        name: "KOReader: Restore note from backup",
        icon: "history",
        checkCallback: (checking) => {
          const file = this.app.workspace.getActiveFile();
          if (!file || file.extension !== "md") return false;
          ...
          if (!Pathing.isAncestor(this.plugin.settings.highlightsFolder, file.path)) return false;
          if (!checking) {
            void this.executeRestoreFromBackup(file);
          }
          return true;
        },
      },
  ...
  async executeFullResetWithConfirm(): Promise<CmdResult> {
    ...
    return this.notifyCmd(this.executeFullReset(), {
      message: () => "Error during reset. Check the developer console for details.",
      timeout: 10000,
    });
  }
  ...
  async executeScan(options?: {
    ...
    const scanResult = await withProgress(
      ...
      async (tick, signal) => {
        ...
        tick.setStatus(`Found ${sdrFilePaths.length} files. Analyzing contents for highlights...`);
        ...
        for await (const res of scanStream) {
          ...
          switch (scanItem.type) {
            ...
            case "empty":
              ...
              break;
            case "error":
              this.log.warn(`Scan failure for ${scanItem.path}: ${scanItem.error}`);
              scanErrors.push({
                path: scanItem.path,
  ...
  private async createOrUpdateScanNote(
    ...
    if (isErr(r)) {
      this.log.error(`Error creating/updating scan report note at ${fullReportPath}:`, r.error);
  ...
  private generateReportContent(
    ...
    const totalFiles = sdrFilePaths.length + filesWithoutHighlights.length + scanErrors.length;
    if (!totalFiles) {
      ...
      content += sdrFilePaths
        .map((metadataFilePath) => {
          const relativePath = usedMount
            ? Pathing.systemRelative(usedMount, metadataFilePath).replace(/\\/g, "/")
            : metadataFilePath;
          return `- \`${relativePath}\``;
        })
        .join("\n");
   ...
   */
  async executeConvertCommentStyle(options?: { signal?: AbortSignal }): Promise<AppResult<void>> {
    ...
    this.log.info(`Conversion complete. Scanned: ${summary.scanned}, Changed: ${summary.changed}`);
    return ok(void 0);
  }
   ...
   */
  public async executeRecheckCapabilities(): Promise<AppResult<{ message?: string }>> {
  ...
  async executeFullReset(): Promise<AppResult<void>> {
    ...
    if (!isErr(listing)) {
      ...
      for (const f of files) {
        ...
        if (isErr(r)) this.log.warn(`Failed to remove data file: ${f}`, r.error);
      }
      for (const d of folders) {
        const r = await this.fs.removePluginDataPath(d);
        if (isErr(r)) this.log.warn(`Failed to remove data folder: ${d}`, r.error);
      }
    } else {
      this.log.warn("Could not list plugin data dir; proceeding with reset.", listing.error);
    }
   ...
   */
  async executeRefreshCurrentNote(file: TFile): Promise<AppResult<{ changed: boolean }>> {
    ...
    if (!latest) {
      this.log.info(`No known source metadata for book ${bookKey}; nothing to refresh.`);
      return ok({ changed: false });
    }
   ...
   */
  async executeFinalizeCurrentNote(file: TFile): Promise<AppResult<{ changed: boolean }>> {
   ...
   */
  async executeCleanupCompletedBooks(): Promise<AppResult<{ scanned: number; changed: number }>> {
    ...
    try {
      await withProgress(
        ...
        0,
        async (tick, signal) => {
          tick.setStatus(`Processing ${completedFiles.length} completed books...`);
  ...
  /**
   * Restores a note from backup using the RestoreBackupModal.
   * @param file - The file to restore
   */
  async executeRestoreFromBackup(file: TFile): Promise<void> {
    try {
      const modal = new RestoreBackupModal(this.app, this.persistence, file);

      await modal.openAndAwaitResult();
      // The modal handles all notices (success/failure) internally
    } catch (error) {
   ...
   */
  private async _convertAllToCurrentStyle(signal?: AbortSignal): Promise<ConversionSummary> {
    const folder = this.plugin.settings.highlightsFolder;
    const summary: ConversionSummary = { scanned: 0, changed: 0 };
  ...
  private async _convertSingleFile(
    ...
    try {
      ...
      if (isErr(parsed)) {
        this.log.warn(`Skipping conversion for ${file.path} due to parsing error.`, parsed.error);
        return err({
          kind: "YamlParseError",
