...
export class DeviceService implements SettingsObserver, Disposable {
  ...
  private findSdrDirectoriesWithMetadataMemoized: (scanPath: string) => Promise<string[]>;
  ...
  constructor(
    ...
    this.getCdbLazy = asyncLazy<ConcurrentDatabase | null>(() => this.createCdbInstance());
  }
  ...
  onSettingsChanged(newSettings: KoreaderHighlightImporterSettings): void {
    if (
      ...
      this.log.info("Environment settings changed, resetting caches and DB connection.");
    ...
    for (let i = 0; i < MAX_WALK_UP; i++) {
      ...
      if (koReaderRoot) {
        const dbPathsToCheck = [
          Pathing.joinSystemPath(koReaderRoot, ".adds/koreader/settings/statistics.sqlite3"),
          Pathing.joinSystemPath(koReaderRoot, "koreader/settings/statistics.sqlite3"),
        ];
  ...
  private async hasSdrFoldersQuickCheck(
    ...
    let checked = 0;
    const checkDir = async (dir: string, depth: number = 0): Promise<boolean> => {
      ...
      for await (const result of this.fs.iterateNodeDirectory(dir)) {
        ...
        if (dirent.isDirectory() && !dirent.name.startsWith(".")) {
          const found = await checkDir(Pathing.joinSystemPath(dir, dirent.name), depth + 1);
  ...
  private async _validateScanPath(): Promise<string | null> {
    ...
    this.log.warn("Configured scan path is not a usable directory:", configured);
    return null;
  }
  ...
  async findSdrDirectoriesWithMetadata(opts?: { signal?: AbortSignal }): Promise<string[]> {
  ...
  private async scan(scanPath: string, opts?: { signal?: AbortSignal }): Promise<string[]> {
    const excluded = new Set(this.settings.excludedFolders.map((e) => e.trim().toLowerCase()));
    ...
    const stream = runPool(
      sdrDirs,
      async (sdrPath) => {
        const metadataFileName = await this.getMetadataFileName(sdrPath, scanPath);
        return metadataFileName ? Pathing.joinSystemPath(sdrPath, metadataFileName) : null;
      },
  ...
  private async getMetadataFileName(dir: string, mountPoint: string): Promise<string | null> {
    ...
    const allow = new Set<string>(
      allowedFileTypes.map((t: string) => t.trim().toLowerCase()).filter(Boolean),
    );
  ...
  private async refreshIfDeviceDbChanged(): Promise<void> {
    ...
    if (mtime && this.dbFileMtimeMs && mtime > this.dbFileMtimeMs) {
      this.log.info("KOReader stats DB changed on device. Reloading in-memory DB.");
  ...
  private createCdbInstance = async (): Promise<ConcurrentDatabase | null> => {
    ...
    if (!filePath) {
      this.log.info("Statistics DB path not found or configured. Stats disabled.");
      return null;
    }

    const bytesRes = await this.fs.readBinaryAuto(filePath);
    if (isErr(bytesRes)) {
      this.log.error(`Failed to read KOReader stats DB bytes: ${filePath}`, bytesRes.error);
      return null;
    }
  ...
  private prepareSessionIndexes(db: Database): void {
    try {
      db.exec("CREATE INDEX IF NOT EXISTS idx_page_stat_book ON page_stat_data(id_book);");
      db.exec("CREATE INDEX IF NOT EXISTS idx_page_stat_start_time ON page_stat_data(start_time);");
    } catch (e) {
      this.log.warn("Failed to create session indexes (continuing without them).", e);
    }
  }
  ...
  ): Promise<Array<{ id: number; md5: string; title: string; authors: string }>> {
    const db = await this.getConcurrentDb();
    if (!db) return [];

    return db.execute(async (database) => {
      const result = statisticsCore.queryBooksByIdentifiers(database, identifiers);
      return result;
    }, false);
  }
}
