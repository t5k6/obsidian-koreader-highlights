...
import type { ExecResult, ExecutorIO, ImportContext, ImportPlan } from "./types";
...
export async function executeImportPlan(
  ...
  switch (plan.kind) {
    ...
    case "CREATE": {
      ...
      const bodyResult = await timed("Render", () => renderNoteBody(lua, io, opts));
      if (isErr(bodyResult)) {
    ...
    case "MERGE": {
      ...
      if (mergeResult.status === "keep-both") {
        ...
        const bodyResult = await timed("Render", () => renderNoteBody(ctx.luaMetadata!, io, opts));
        if (isErr(bodyResult)) {
          io.log.error("Failed to render note body after keep-both choice", bodyResult.error);
          return { status: "skipped", file: null, warnings: [] };
        }

        const prepared = prepareNoteCreation(ctx.luaMetadata!, bodyResult.value, io.settings);
        ...
        if (isErr(created)) {
          io.log.error("Failed to create note after keep-both choice", created.error);
          return { status: "skipped", file: null, warnings: [] };
        }
      ...
      if (mergeResult.status === "skipped") return { status: "skipped", file: null, warnings: [] };
...
async function renderNoteBody(
  ...
  try {
    ...
    return err({
      ...
      message: `Body rendering failed unexpectedly: ${e instanceof Error ? e.message : String(e)}`,
    } as AppFailure);
  }
}
