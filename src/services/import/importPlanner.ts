...
import type { DuplicateMatch, DuplicateScanResult, LuaMetadata } from "src/types";
...
function determineImportAction(
  ...
  const inHighlightsFolder =
    currentFolder === "" ? isDirectlyInRoot : matchPath.startsWith(`${currentFolder}/`);
...
export async function planImport(
  ...
  const statsResult = await timed("Stats", () => io.fs.getNodeStats(ctx.metadataPath));
  ...
  await timed(
    ...
    async () => {
      ...
      if (ctx.forceNote) {
        ...
        const scan = await io.dupFinder.findBestMatch(ctx.luaMetadata, degradedScanCache);
  ...
  if (!ctx.forceReimport && ctx.stats) {
    ...
    if (!shouldProcess) {
      ...
      const { missingPaths, diagnostics: missingTargetDiagnostics } = await collectMissingTargets(
        targetPaths,
        io.fs,
        opts?.signal,
      );
      ...
      reasonToSkip = missingNotePaths.length > 0 ? "unchanged-but-missing-note" : "unchanged";
    }
  }
