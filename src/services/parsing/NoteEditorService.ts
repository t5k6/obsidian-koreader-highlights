...
export class NoteEditorService implements FileMetadataExtractor {
  ...
  public async editFile(
    ...
    return this.editQueue.run(file.path, async () => {
      ...
      if (isErr(parseResult)) {
        this.log.error("editFile: failed to parse current content, aborting edit.", {
          file: file.path,
      ...
      const fmEqual = noteCore.areFrontmattersEqual(currentDoc.frontmatter, nextDoc.frontmatter);
      const bodyEqual = currentDoc.body === nextDoc.body;
      if (options.skipIfNoChange && fmEqual && bodyEqual) {
        return ok({ changed: false, file });
      }

      const newContent = noteCore.reconstructNoteContent(nextDoc.frontmatter, nextDoc.body);
  ...
  public async parseFile(
    file: TFile,
    signal?: AbortSignal,
  ): Promise<Result<{ frontmatter: Record<string, unknown>; body: string }, ParseFailure>> {
    throwIfAborted(signal);
    const readResult = await this.fs.readVaultText(file.path);

    if (isErr(readResult)) {
      this.log.warn(`Failed to read file during parseFile: ${file.path}`, readResult.error);
      return err({
        kind: "YamlParseError",
   ...
   */
  public async extractMetadata(file: TFile, signal?: AbortSignal): Promise<BookMetadata | null> {
    try {
      throwIfAborted(signal);
  ...
  /**
   * [STATEFUL SHELL] Extracts a validated note UID, prioritizing the metadata cache.
   */
  public async extractUid(file: TFile, signal?: AbortSignal): Promise<string | undefined> {
    try {
      throwIfAborted(signal);
      const fm = await this.getFrontmatterFromFile(file, signal);
      return extractUidFromFrontmatter(fm);
    } catch (e) {
      if (!isAbortError(e)) {
        this.log.warn(`extractUid failed for ${file.path}`, e);
      }
      return undefined;
    }
  }
  ...
  public async setFrontmatterFields(file: TFile, patch: Record<string, unknown>): Promise<void> {
