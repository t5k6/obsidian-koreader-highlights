import { normalizePath, parseYaml, type Vault } from "obsidian";
...
export class TemplateManager implements SettingsObserver {
  ...
  public customTemplateFilesCache: { path: string; name: string }[] | null = null;
  public customTemplateFilesLoading = false;
  ...
  constructor(
    ...
    this.compiledTemplateCache = cacheManager.createLru("template.compiled", 10);
    this.filterPipelineCache = cacheManager.createLru("template.filterPipelines", 200);
    ...
    this.plugin.registerEvent(
      this.plugin.app.vault.on("modify", (file) => {
        const { useCustomTemplate, selectedTemplate } = this.plugin.settings.template;
  ...
  public onSettingsChanged(
    ...
    const templateChanged =
      oldSettings.template.useCustomTemplate !== newSettings.template.useCustomTemplate ||
      oldSettings.template.selectedTemplate !== newSettings.template.selectedTemplate ||
      oldSettings.template.templateDir !== newSettings.template.templateDir ||
      oldSettings.template.noteQuotingMode !== newSettings.template.noteQuotingMode;

    if (templateChanged) {
      this.cacheManager.clear("template.*");
      this.customTemplateFilesCache = null;
  ...
  public async loadBuiltInTemplates(): Promise<void> {
    ...
    try {
      const rawTemplates: Record<string, string> = JSON.parse(KOREADER_BUILTIN_TEMPLATES);
      for (const id in rawTemplates) {
        ...
        if (fmContent) {
          try {
            ...
            if (typeof (fm as any).description === "string" && (fm as any).description.trim()) {
  ...
  public async loadTemplateResult(): Promise<Result<string, TemplateFailure>> {
    ...
    const rawContent = useCustomTemplate ? stripFrontmatter(contentRes.value) : contentRes.value;
   ...
   */
  public async getCompiledTemplateResult(): Promise<Result<CompiledTemplate, TemplateFailure>> {
  ...
  async ensureTemplates(): Promise<Result<void, AppFailure>> {
    ...
    const writePromises = Array.from(this.builtInTemplates.values()).map(async (template) => {
  ...
  private _getBuiltInTemplateContent(id: string): Result<string, TemplateFailure> {
