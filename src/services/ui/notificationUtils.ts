...
import type { AppFailure, AppResult, FileSystemFailure } from "src/lib/errors/types";
...
export async function notifyOnError<T, E = AppFailure>(
  ...
  if (isErr(res)) {
    let msg: string;
    if (ops?.message) {
      msg = typeof ops.message === "function" ? ops.message(res.error as any) : ops.message;
    } else if (!!res.error && typeof res.error === "object" && "kind" in (res.error as any)) {
