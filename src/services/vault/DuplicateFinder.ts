...
import { getOptimalConcurrency, isAbortError, runPool, throwIfAborted } from "src/lib/concurrency";
...
import { buildBookKey, getStrongIdentifiers, isUniqueMd5 } from "src/lib/metadata/identity";
...
export class DuplicateFinder implements SettingsObserver {
  ...
  constructor(
    ...
    this.plugin.registerEvent(this.vault.on("rename", () => this.invalidateCaches()));
    this.plugin.registerEvent(this.vault.on("delete", () => this.invalidateCaches()));
  }
  ...
  public async findBestMatch(
    ...
    if (directCandidates.length > 0) {
      const match = await this._findAndAnalyzeBest(directCandidates, luaMetadata);
      if (match) {
        this.log.info(`Found instant duplicate match via expected filename: ${match.file.path}`);
    ...
    const useIndex =
      this.localIndexService.isReady() && !this.localIndexService.isRebuildingIndex();

    if (useIndex) {
      const indexCandidates = await this._findViaIndex(bookKey);
      if (indexCandidates.length > 0) {
        const match = await this._findAndAnalyzeBest(indexCandidates, luaMetadata);
        if (match) {
          return { match, confidence: "full" };
        }
      }
    ...
    this.log.warn("No matches found via priority methods. Falling back to degraded vault scan.");
  ...
  public async scanVaultForKey(
    ...
    try {
      const result = await this._runDegradedScan(bookKey, docProps, controller.signal);
      ...
      if (controller.signal.aborted) {
        return {
          files: result.kind === "complete" ? result.items : result.partialItems,
          timedOut: true,
        };
      }
  ...
  public async analyzeCandidateFile(
    ...
    const snapRes = expectedUid ? await this.persistence.readSnapshotById(expectedUid) : null;
  ...
  private async _runDegradedScan(
    ...
    try {
      for await (const file of fileStream) {
        ...
        if (isPotentialMatch(fm, file.basename, bookKey, expectedFilenameKeys)) {
          matches.push(file);
        }
      }
    } catch (e) {
  ...
  private async _findViaDirectProbe(luaMetadata: LuaMetadata): Promise<TFile[]> {
    try {
  ...
  private async _findViaIndex(bookKey: string): Promise<TFile[]> {
    try {
      ...
      return paths.map((p) => this.vault.getAbstractFileByPath(p)).filter(isTFile);
    } catch (e) {
  ...
  private async _findAndAnalyzeBest(
    ...
    const stream = runPool(
      candidateFiles,
      (file) => this.analyzeCandidateFile(file, luaMetadata.annotations, luaMetadata),
    ...
    const [best] = sortDuplicateMatches(analyses, this.plugin.settings.highlightsFolder);
    return best ?? null;
  }
  ...
  public onSettingsChanged(
    ...
    if (!oldSettings || newSettings.highlightsFolder !== oldSettings.highlightsFolder) {
  ...
  private async _findViaUniqueMd5(md5: string): Promise<TFile[]> {
    try {
      const sourceRows = await this.localIndexService.getImportSourcesByMd5(md5);
      const candidates: TFile[] = [];

      for (const row of sourceRows) {
        if (row.book_key) {
          const paths = await this.localIndexService.findExistingBookFiles(row.book_key);
          const files = paths.map((p) => this.vault.getAbstractFileByPath(p)).filter(isTFile);
