...
export class MergeHandler {
  ...
  async handleDuplicate(
    ...
    switch (strategy) {
      ...
      case "prompt-user":
        return this.executeWithUserPrompt(analysis, bodyProvider, session, message);
  ...
  private async executeAutoMerge(
    ...
    const res = await this._performMerge(analysis.file, newBody, analysis.luaMetadata, "merge");
    if (isErr(res)) {
  ...
  private async executeWithUserPrompt(
    ...
    const promptResult = buildPromptMessage(analysis, this.plugin.settings.highlightsFolder);
    ...
    switch (choice) {
      case "replace": {
        ...
        if (isErr(result)) {
          this.log.error(`Replace failed for ${analysis.file.path}`, result.error);
  ...
  private async _performMerge(
    ...
    return this.mergeQueue.run(`merge:${file.path}`, async () => {
      ...
      const baseContent = isErr(baseContentResult) ? null : baseContentResult.value;

      const compiledResult = await this.templateManager.getCompiledTemplateResult();
