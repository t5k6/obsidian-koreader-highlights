...
import { isMarkdownFile, isTFile, isTFolder } from "src/lib/obsidian/typeguards";
...
import type { KoreaderHighlightImporterSettings, SettingsObserver } from "src/types";
...
export class IndexCoordinator implements SettingsObserver {
  ...
  public debouncedHandleMetadataChange!: Debouncer<[TFile, string, CachedMetadata], void>;
  ...
  constructor(
    ...
    this.persistIndexDebounced = debounce(() => void this.flushIndex().catch(() => {}), 1500, true);
    this.debouncedHandleMetadataChange = debounce(
      (file: TFile, data: string, cache: CachedMetadata, signal?: AbortSignal) =>
  ...
  private async writeBatchToIndex(batch: import("src/types").BookMetadata[]): Promise<void> {
    for (const { key, title, authors, vaultPath } of batch) {
      await this.indexRepo.upsertBookWithInstance({ key, id: null, title, authors }, vaultPath);
    }
  }
  ...
  public onSettingsChanged(
    ...
    if (!oldSettings || oldSettings.highlightsFolder !== newSettings.highlightsFolder) {
    ...
    if (isTFile(file)) {
      ...
      if (
        ...
        if (uid && typeof uid === "string") {
          ...
          void this.persistence.deleteSnapshotIfOrphaned(uid, this.settings.highlightsFolder);
        }
      }

      const key = await this.findKeyByVaultPath(pathToDelete);
      const sourcePath = await this.indexRepo.deleteNoteAndResetSource(pathToDelete);
      if (sourcePath) {
        this.log.info(
  ...
  private async _handleMetadataChange(
    ...
    if (this.shouldSkipMetadataUpdate(file.path)) {
      this.log.info(`Skipping metadata update for recently imported file: ${file.path}`);
      return;
    }
    ...
    try {
      ...
      if (!metadata) {
        const { changed, oldKey } = await this.indexRepo.deleteInstanceForFile(file.path);
        if (changed && oldKey) {
          this.pathCache.delete(oldKey);
        }
      } else {
        const { changed, oldKey, newKey } = await this.indexRepo.upsertFromMetadata(file.path, {
          key: metadata.key,
  ...
  public async getImportSourceSafe(path: string): Promise<AppResult<ImportSourceRow | null>> {
    return wrapResult(
      async () => {
        await this.indexDb.whenReady();
        return this.indexRepo.getByPath(path);
  ...
  public async recordImportFailure(path: string, error: unknown): Promise<void> {
    await this.indexRepo.recordImportFailure(path, error);
  }

  public async recordImportFailureSafe(path: string, error: unknown): Promise<AppResult<void>> {
    return wrapResult(
  ...
  public async upsertBook(
    ...
    await this.indexRepo.upsertBookWithInstance({ key, id, title, authors }, vaultPath);
    this.pathCache.delete(key);
  }
  ...
  public async getImportSourcesByMd5(
    md5: string,
  ): Promise<Array<{ source_path: string; book_key: string | null; md5: string | null }>> {
    await this.indexDb.whenReady();
  ...
  public async flushIndex(): Promise<void> {
    ...
    try {
      ...
      new Notice(`Failed to save index: ${e instanceof Error ? e.message : e}`, 8000);
      throw e;
    }
  }
  ...
  public isRebuildingIndex(): boolean {
    return this.indexDb.getState() === "in_memory" && this.indexDb.isRebuilding();
  }
  ...
  public isReady(): boolean {
    ...
    if (this.indexDb.getState() === "in_memory" && this.indexDb.isRebuilding()) {
      return false;
    }
    return true;
  }
