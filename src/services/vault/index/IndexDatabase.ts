...
export type RebuildPhase = "idle" | "rebuilding" | "complete" | "failed" | "cancelled";
...
export class IndexDatabase {
  ...
  public onRebuildStatus(listener: (status: RebuildStatus) => void): () => void {
  ...
  private async ensureReady(): Promise<void> {
    if (this.idxDb || this.initializing) return this.initializing ?? Promise.resolve();
    ...
    this.initializing = (async () => {
      try {
        ...
        this.log.warn("IndexDatabase persistent open failed; falling back to in-memory", e);
      }
