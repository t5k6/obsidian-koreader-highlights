...
export const QueryBuilders = {
  ...
  upsertBook: (key: string, id: number | null, title: string, authors: string) => ({
  ...
  upsertImportSourceFailure: (sourcePath: string, mtime: number, size: number, error: string) => ({
  ...
  if (start >= 2 && !tableExists(db, "import_source")) {
    try {
      ...
      db.run(`
		    ...
		    `);
      db.run("CREATE INDEX IF NOT EXISTS idx_import_source_book_key ON import_source(book_key);");
      db.run("CREATE INDEX IF NOT EXISTS idx_import_source_md5 ON import_source(md5);");
      db.run("COMMIT;");
