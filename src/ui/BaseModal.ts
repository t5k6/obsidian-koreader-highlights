import { type App, ButtonComponent, type EventRef, Modal, Scope } from "obsidian";
...
export class BaseModal<T = void> extends Modal {
   ...
   */
  protected registerShortcut(modifiers: string[], key: string, handler: () => void): void {
    ...
    this.scope.register(modifiers as Parameters<Scope["register"]>[0], key, wrappedHandler);
  ...
  protected handleEnter(): void {
    ...
    const primaryButton = this.contentEl.querySelector<HTMLButtonElement>(".cta:not([disabled])");
    if (primaryButton) {
      primaryButton.click();
      return;
    }
  ...
  public createButtonRow(
    ...
    buttons.forEach((spec) => {
      const btn = new ButtonComponent(container).setButtonText(spec.text).onClick(spec.onClick);
  ...
  private handleFocusTrap(event: KeyboardEvent): void {
    ...
    const activeElement =
      this.app.workspace.activeLeaf?.view.containerEl.doc.activeElement ?? document.activeElement;
