...
export class FrontmatterFieldModal extends BaseModal<FrontmatterSettings> {
  ...
  protected renderContent(contentEl: HTMLElement): void {
    ...
    for (const field of this.fieldOptions) {
      this.fieldStates[field.id] = this.currentSettings.disabledFields.includes(field.id);
    }
      ...
      button
        ...
        .onClick(() => {
          ...
          if (!/^[a-zA-Z0-9_-]+$/.test(newFieldValue)) {
            new Notice("Field names can only contain letters, numbers, hyphens and underscores");
            return;
          }
    ...
    const firstToggle = contentEl.querySelector('input[type="checkbox"]') as HTMLElement;
