...
export class StatusBarManager extends Component {
  ...
  private refreshableCache: IterableCache<string, boolean> | Map<string, boolean> | null = null;
  ...
  override onload(): void {
    ...
    const diContainer = (this.plugin as unknown as { diContainer?: any }).diContainer;
    const cacheManager = diContainer?.resolve?.(CacheManager) as CacheManager | undefined;
    if (cacheManager) {
      this.refreshableCache = cacheManager.createLru("statusBarRefreshable", 500);
    } else {
    ...
    this.registerDomEvent(this.statusBarItem, "click", () => this.handleStatusBarClick());
    this.registerEvent(this.app.workspace.on("active-leaf-change", () => this.updateVisibility()));
  ...
  private async handleStatusBarClick(): Promise<void> {
    ...
    await runAsyncAction(
      this.statusBarItem,
      async () => {
        const res = await notifyOnError(this.commandManager.executeRefreshCurrentNote(file), {
