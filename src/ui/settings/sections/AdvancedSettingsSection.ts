...
export class AdvancedSettingsSection extends SettingsSection {
  protected renderContent(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc("Controls verbosity of logs. 'None' is off, 'Info' is most verbose.")
      .addDropdown((dd) => {
        ...
        dd.setValue(String(settings.logLevel)).onChange(async (v) => {
          settings.logLevel = parseInt(v, 10) as KoreaderHighlightImporterSettings["logLevel"];
          this.debouncedSave();
        });
      });
    ...
    new Setting(containerEl)
      ...
      .setDesc("Makes the plugin forget which books have been imported. Notes are NOT deleted.")
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            ...
            new Notice("Import status has been reset. Run an import to re-process all books.");
          }),
      );
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dd) => {
        ...
        dd.setValue(settings.commentStyle).onChange(async (v) => {
          settings.commentStyle = v as KoreaderHighlightImporterSettings["commentStyle"];
          await this.saveAndReload();
        });
      });
    ...
    new Setting(containerEl)
      ...
      .addButton((btn) =>
        btn
          ...
          .setWarning()
          .onClick(async () =>
            runAsyncAction(btn, () => this.plugin.triggerConvertCommentStyle(), {
    ...
    new Setting(containerEl)
      ...
      .addButton((btn) =>
        btn
          ...
          .setWarning()
          .onClick(async () =>
            runAsyncAction(btn, () => this.plugin.triggerCleanupCompletedBooks(), {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Backups are created automatically before KOReader notes are changed. To restore one, right-click a highlight note or use the Command Palette command 'KOReader: Restore note from backup'. Set to 0 to disable the limit.",
      )
      .addText((text) => {
        ...
        text.inputEl.max = "20";
        text.inputEl.step = "1";
        text.setValue(String(settings.maxBackupsPerNote)).onChange(async (v) => {
