...
export class CoreSettingsSection extends SettingsSection {
  protected renderContent(container: HTMLElement): void {
    ...
    const scanPathSetting = new Setting(container)
      ...
      .setDesc("Root folder to scan for KOReader .sdr directories (usually your device mount).")
      .addText((text) => {
        text
    ...
    const setValidationMessage = (
      message: string,
      kind: "pending" | "success" | "error" | "none",
    ) => {
      if (kind === "error") {
        renderValidationError(validationEl, message);
        validationEl.style.color = "var(--text-error)";
        return;
      }
      validationEl.empty();
      validationEl.setText(message);
      validationEl.style.color =
        kind === "success" ? "var(--text-success)" : kind === "pending" ? "var(--text-muted)" : "";
    };
    ...
    const validatePath = async (path: string) => {
      ...
      setValidationMessage("Checking path…", "pending");

      const deviceService = (this.plugin as any).diContainer?.resolve(DeviceService);
      if (!deviceService) {
        setValidationMessage("Unable to validate path.", "error");
      ...
      try {
        ...
        if (result.valid) {
          ...
          setValidationMessage(`Valid: ${statsMsg}${sdrMsg}`, "success");
        } else {
          setValidationMessage("Path not found or does not contain KOReader data.", "error");
        }
      } catch (error) {
    ...
    attachBrowseIconButton({
      ...
      inputEl: scanPathSetting.controlEl.querySelector("input") as HTMLInputElement,
      icon: "folder-open",
      tooltip: "Browse…",
    ...
    attachBrowseIconButton({
      ...
      inputEl: statsSetting.controlEl.querySelector("input") as HTMLInputElement,
      icon: "folder-open",
      tooltip: "Browse…",
      onPick: () =>
        pickFile("Select KOReader statistics database", {
          filters: [{ name: "SQLite DB", extensions: ["sqlite3", "sqlite", "db"] }],
        }),
      onSave: async (v) => {
        this.plugin.settings.statsDbPathOverride = v;
        this.debouncedSave();
    ...
    new Setting(container)
      ...
      .addText((text) => {
        ...
        text.setValue(String(this.plugin.settings.scanTimeoutSeconds ?? 8)).onChange(async (v) => {
