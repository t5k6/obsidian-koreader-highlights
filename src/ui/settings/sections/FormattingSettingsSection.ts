...
export class FormattingSettingsSection extends SettingsSection {
  protected renderContent(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(settings.useCustomFileNameTemplate).onChange(async (value) => {
    ...
    if (settings.useCustomFileNameTemplate) {
      ...
      const validate = (val: string) => {
        ...
        validationEl.style.color = errors.length > 0 ? "var(--text-error)" : "var(--text-muted)";
      };
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) => {
        dropdown
          ...
          .onChange(async (value) => {
            settings.frontmatter.keywordsAsTags = value as "none" | "duplicate" | "replace";
            this.debouncedSave();
          });
      });
    ...
    new Setting(containerEl)
      ...
      .setDesc("When an author is not found, use 'Unknown Author' instead of omitting the field.")
      .addToggle((t) =>
        t.setValue(settings.frontmatter.useUnknownAuthor).onChange(async (v) => {
    ...
    if (settings.silentImport) {
      const strategySetting = new Setting(containerEl)
        ...
        .setDesc("Action to take for all duplicates when running in silent mode.")
        .addDropdown((d) =>
          d
