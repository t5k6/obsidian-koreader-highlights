...
export class TemplateSettingsSection extends SettingsSection {
  ...
  protected renderContent(containerEl: HTMLElement): void {
        ...
        toggle.setValue(template.useCustomTemplate).onChange(async (value) => {
          ...
          if (value) {
            ...
            template.selectedTemplate =
              this.templateManager.customTemplateFilesCache?.[0]?.path ?? "";
          } else {
    ...
    if (template.useCustomTemplate) {
      new Setting(containerEl)
        ...
        .addSearch((search) => {
          ...
          search.inputEl.addEventListener("blur", async () => {
            ...
            this.templateManager.customTemplateFilesCache = null;
      ...
      const templates: { path: string; name: string }[] =
        this.templateManager.customTemplateFilesCache || [];
      const desc = this.templateManager.customTemplateFilesLoading
        ? "Loading templates…"
        : templates.length > 0
      ...
      dropdownSetting.addButton((btn) => {
        ...
        btn
          ...
          .onClick(() =>
            runAsyncAction(btn, () => this.showPreviewModal(template.selectedTemplate, true), {
              inProgress: "Opening...",
            }),
          );
      });
      ...
      dropdownSetting
        .addButton((btn) =>
          btn.setButtonText("Preview").onClick(() =>
            runAsyncAction(btn, () => this.showPreviewModal(template.selectedTemplate, false), {
              inProgress: "Opening...",
            }),
          ),
        )
  ...
  private ensureTemplateCache(): void {
    if (this.templateManager.customTemplateFilesCache !== null) return;
    this.templateManager.customTemplateFilesCache = [];
    this.templateManager.customTemplateFilesLoading = true;
    void this.loadTemplateCache();
  }

  private async loadTemplateCache(): Promise<void> {
    const folder = Pathing.toVaultPath(this.plugin.settings.template.templateDir);
    try {
      const scan = await this.templateManager.fs.getFilesInFolder(folder, {
        extensions: ["md", "txt"],
        recursive: true,
      });
      this.templateManager.customTemplateFilesCache = scan.files.map((f) => ({
        path: f.path,
      ...
      this.templateManager.customTemplateFilesLoading = false;
    }
  ...
  private async handleCreateFromBuiltIn(): Promise<void> {
    const selectedBuiltInId = this.plugin.settings.template.selectedTemplate || "default";
    const builtInTemplate = this.templateManager.builtInTemplates.get(selectedBuiltInId);
    ...
    const templateDir = Pathing.toVaultPath(this.plugin.settings.template.templateDir);
    ...
    const existsResult = await this.templateManager.fs.vaultExists(newTemplatePath);
    ...
    if (existsResult.value) {
      new Notice(`Error: A template named "${finalFileName}" already exists in ${templateDir}.`);
      return;
    }
    ...
    try {
      ...
      console.error(e);
      new Notice("Error creating template file. Check the console for details.");
    }
  }
}
