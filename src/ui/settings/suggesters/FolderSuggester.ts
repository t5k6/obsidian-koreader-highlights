...
export class FolderSuggest extends TextInputSuggest<ScoredFolder> {
  getSuggestions(inputStr: string): ScoredFolder[] {
    ...
    for (const folder of folders) {
      ...
      if (match) {
        const flat: number[] = [];
        for (const range of match.matches) for (let k = range[0]; k < range[1]; k++) flat.push(k);
        results.push({ folder, score: match.score, matchIndices: flat });
      }
    }

    results.sort((a, b) =>
      b.score !== a.score ? b.score - a.score : a.folder.path.length - b.folder.path.length,
    );
  ...
  private highlightMatches(text: string, indices: number[], el: HTMLElement): void {
