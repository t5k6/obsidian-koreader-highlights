...
class Logger {
	...
	/* ----------- configuration ----------- */
	setLevel = (lvl: DebugLevel) => {
		this.level = lvl;
	};

	enableFileSink(enable: boolean, vault?: Vault, dir = "KOReader/logs") {
	...
	/* ----------- public logging API ----------- */
	info = (...a: unknown[]) => this.emit("INFO", DebugLevel.INFO, a);
	warn = (...a: unknown[]) => this.emit("WARN", DebugLevel.WARN, a);
	error = (...a: unknown[]) => this.emit("ERROR", DebugLevel.ERROR, a);

	/* ----------- internals ----------- */
	private emit(tag: string, lvl: DebugLevel, args: unknown[]) {
		if (this.level <= lvl) {
